...
window.onload = async function () {
    ...
    var parts = await getOwnedItemsFromEvent(window.accounts[1], 'TransferPartOwnership')
    console.log(parts)
    ...
    document.getElementById("build-car").addEventListener("click", function () {
        ...
        if (serial != "") {
            ...
            window.pm.methods.buildProduct(serial, "Car", creation_date, part_array).send({ from: window.accounts[1], gas: 2000000 }, function (error, result) {
                if (error) {
                    console.log(error)
                } else {
                    console.log("Car created")
                    //Add hash to car owned list
                    var car_sha = web3.utils.soliditySha3(window.accounts[1], web3.utils.fromAscii(serial),
    ...
    document.getElementById("car-change-ownership-btn").addEventListener("click", function () {
        ...
        if (hash_element != undefined) {
            ...
            if (to_address != "") {
                window.co.methods.changeOwnership(1, hash_element.textContent, to_address).send({ from: window.accounts[1], gas: 100000 }, function (error, result) {
                    if (error) {
                        console.log(error)
                    } else {
