...
contract ChangeOwnership {
    ...
    function pushPart(bytes memory manufactory, string memory serial_number, string memory part_type, string memory creation_date) public {
        //Create hash for data and check if it exists. If it doesn't, create the part and return the ID to the user
        bytes32 part_hash = concatenateInfoAndHash(manufactory, serial_number, part_type, creation_date);
        emit parthash(part_hash);

        require(parts[part_hash].manufacturer == address(0), "Part ID already used");

        Part memory new_part = Part(manufactory, serial_number, part_type, creation_date);
        parts[part_hash] = new_part;
        parts_list[parts_cnt] = part_hash;
        parts_cnt +=1;
        addOwnership(uint(OperationType.PART), part_hash);
    }
